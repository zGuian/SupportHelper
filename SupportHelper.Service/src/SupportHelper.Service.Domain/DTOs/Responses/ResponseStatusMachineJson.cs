namespace SupportHelper.Service.Domain.DTOs.Responses
{
    public record ResponseStatusMachineJson
    {
        ...
        public IEnumerable<ResponseNetworkBoard> NetworkBoards { get; init; } = [];
        public required string UpTime { get; init; }
        public required string LastUpdate { get; init; }

        public ResponseStatusMachineJson() { }

        public static ResponseStatusMachineJson Create(bool isConnected, string hostname, bool sgpIsRunning, string currentUsername,
            string domainName, string operationalSystem, IEnumerable<ResponseNetworkBoard> networkBoardResponses, string upTime, string lastUpdate)
        {
